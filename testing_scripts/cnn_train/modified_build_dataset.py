...
subdirs = [dirs for dirs in os.listdir(imgdir)] # if os.path.isdir(os.path.join(imgdir, dirs))]
print(subdirs)
...
for s in subdirs:
    os.chdir(s)
    #print(os.path.join(imgdir,s))
    # files = [filz for filz in os.listdir(s)] # if fnmatch.fnmatch(filz, '^[01234567890.]{10,13}\-[\w]{0,64}\-[FNB][LSR].png$')]
    
    #files = os.listdir(os.path.join(imgdir,s))
    # this join was not needed in my test scripts...
    files = os.listdir(s)
    print(files)
